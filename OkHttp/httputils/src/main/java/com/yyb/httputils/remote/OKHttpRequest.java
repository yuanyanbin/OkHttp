...
import com.yyb.httputils.R;
import com.yyb.httputils.util.LogUtil;
import com.yyb.httputils.util.MD5;
...
import com.yyb.httputils.constant.Constants;
import com.yyb.httputils.util.NetConnectUtil;

import static com.yyb.httputils.constant.Constants.TYPE_GET;
import static com.yyb.httputils.constant.Constants.TYPE_POST_FORM;
import static com.yyb.httputils.constant.Constants.TYPE_POST_JSON;
        ...
        try {
            ...
            switch (success) {
                case 1000://操作成功
                    ...
                    break;
                case 2000://操作失败（提示给用户）
                    failedCallBack(2000, errorMsg, callBack);
                    break;
                case 2003: //微信授权信息有误
                    failedCallBack(2003, errorMsg, callBack);
                    break;
                case 3000://操作失败异常（不需要提示给用户）
                    failedCallBack(3000, context.getResources().getString(R.string.tips_request_server_fail), callBack);
                    break;
                case 4000://token失效,跳转到登录界面
                    final String token_errorMsg = "登录已失效，请重新登录！";
                    ...
                    break;
                case 5000://签名验证不通过
                    failedCallBack(5000, context.getResources().getString(R.string.tips_request_server_fail), callBack);
                    break;
                case 5001://Header参数错误：指的是header传递的值不存在或者不支持时 返回此码
                    failedCallBack(5001, context.getResources().getString(R.string.tips_request_server_fail), callBack);
                    break;
                default:
                    //操作失败（提示给用户）
